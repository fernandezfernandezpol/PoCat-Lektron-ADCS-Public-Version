...
% w0 = 300*[0;-OrbRate(a0);0]; % .001
% w0 = [100*OrbRate(a0);-300*OrbRate(a0);-175*OrbRate(a0)]; % .001
w0 = [50*OrbRate(a0);-180*OrbRate(a0);-85*OrbRate(a0)]; % .001
% w0=[0.7;0.47;-0.65];
b0 = 20000;
x = [r0;v0;q0;w0;b0];
PltOrbit( el, d.jD0 );
...
d.maxmoment = [0.032;0.024;0.032]; %amp*m^2
d.k=10^-5;
...
for k = 1:nSim
    ...
    d.bdot = (d.fieldBODY-d.fieldBODYbefore)/(dT);
    magmoment(1,1) = -d.maxmoment(1,1)*sign(d.bdot(1,1));
    magmoment(2,1) = -d.maxmoment(2,1)*sign(d.bdot(2,1));
    magmoment(3,1) = -d.maxmoment(3,1)*sign(d.bdot(3,1));
    % magmoment= -d.k*d.bdot;
    ...
    d.dipole     = magmoment; % Amp-turns m^2
...
Plot2D( tP, rad2deg(xPlot(11:13,:)), tL, {yL{11:13}}, 'CubeSat Attitude Rate (degrees/s)' );
