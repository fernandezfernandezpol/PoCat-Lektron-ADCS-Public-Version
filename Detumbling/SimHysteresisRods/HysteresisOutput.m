...
persistent xP zP tP kF hWaitbar tStart
...
if isempty(flag)
  ...
  %   fprintf('%d0%% finished\n',kF);
  % end
  progress = t(end)/d.end;
  if ishandle(hWaitbar)
    elapsedTime = toc(tStart);
    elapsedStr = char(duration(0, 0, elapsedTime, 'Format', 'hh:mm:ss'));
    waitbar(progress, hWaitbar, sprintf(['Simulation in progress... %3.0f%%' ...
        '   |   Elapsed: %s'], progress*100, elapsedStr));
  end
elseif strcmp(flag,'init')
  ...
  kF = 0;
  hWaitbar = waitbar(0, 'Simulation in progress...', ...
      'Name', 'Hysteresis Rods for PQ Simulation');
  tStart = tic;
