...
clear g; clear u; clear p; clear d; close all; clear all;
...
w    = [10*OrbRate(a);-5*OrbRate(a);-4*OrbRate(a)];                 % Angular rate of spacecraft
...
for k = 1:nSim
  ...
  %-----------------------------------------------------------------
  bField_I   = QForm( qECIToBody, BDipole( x(1:3), d.jD0 + t/86400) );
  ...
  bField(:,k)=bField_I+magNoise'+magBias';
...
%% Calculate the dinamycs
[d,x,p,hRWA,angleError(k+1),torque] =LatLongFunction(x,d,p,jD,lat,long);
  ...
  xPlot(:,k+1) = [x;power;torque;angleError(k+1);bField(:,k);hRWA];
  t            = t + dT;
...
Plot2D(t,angleError);
title('Angle Error between Z and normal(deg)');
...
figure
plot(t, Temp -  273.15); xlabel('Time [hours]'); ylabel('Average Temperature [ºC]');
grid
...
PlotSatAttitude(xPlot,lat,long)
