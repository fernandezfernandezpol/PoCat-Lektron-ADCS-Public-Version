...
v    = [0;v.*cosd(i);v.*sind(i)];                 % Velocity vector - polar orbit
q    = [1;0;0;0];               % Quaternion
w    = [0.9*OrbRate(a);-1*OrbRate(a);-1*OrbRate(a)];                 % Angular rate of spacecraft
...
for k = 1:nSim
 ...
 %% Nadir Pointing
[d,angleError(k+1),angleError2(k+1),x,time,hRWA,p,p2,torque] =NadirFunction(x,d,p,p2,time);
...
xlabel('Time (min)'); ylabel('degrees º');
...
figure
plot(t, theta); xlabel('Time hours]'); ylabel('Angle antenna boresight - nadir [deg]');
grid
%% Thermal Control
...
figure
plot(t, Temp -  273.15); xlabel('Time [days]'); ylabel('Average Temperature [ºC]');
grid
