...
w    = pi/180*(2*[rand;rand;rand]-1)*3;                 % Angular rate of spacecraft
c    = [0;0;0];                 % Reaction wheel rates
b    = FullBatery;                  % Battery state of charge (J = Wh*3600)
...
for k = 1:nSim
  ...
  %magNoise= [normrnd(0,Sigma_Noise(1)),normrnd(0,Sigma_Noise(2)),normrnd(0,Sigma_Noise(3))];
...
if norm(w_read) > norm(w_satbilized*[1;1;1])&& Mode.Detumbling==1 
    d.tRWA=[0;0;0];
    [d,x,p] =DetumblingFunction(x,d,p,bField(:,k),dT,Sensors);  
    d.kWheels=[];
elseif  norm(w_read) < norm(w_satbilized*[1;1;1])&& Mode.Detumbling==1 
     Mode.Detumbling=0;
     Mode.Nadir=1;  
elseif Mode.Detumbling==0
    if x(17)<b_min
        Mode.SunPointing=1;
        [d,angleError3(k+1),x,p] =SunPointingFunction(x,d,p,jD);
    elseif x(17)>b_top && Mode.SunPointing==1
        Mode.SunPointing=0;
        Mode.Nadir=1;
    elseif Mode.Nadir==1
        [d,angleError1(k+1),angleError2(k+1),x,time,hRWA,p,p2,torque] =NadirFunction(x,d,p,p2,time);
        
    elseif Mode.LatLongPointing==1;
        lat=40;
        long=2;
        [d,x,p,hRWA ,angleError4(k+1),torque] =LatLongFunction(x,d,p,jD,lat,long);
    elseif ZenithPointing==1;
        [d,angleError5(k+1),angleError6(k+1),x,time,hRWA,p,p2,torque] =ZenithFunction(x,d,p,p2,time);
    
    end
end
 angleError1(k+1)   = acos(Dot(p.eci_vector,QTForm(qECIToBody,p.body_vector)))*radToDeg;
  angleError2(k+1)  = acos(Dot(p2.eci_vector,QTForm(qECIToBody,p2.body_vector)))*radToDeg;
angleError3(k+1)   = acos(Dot(p3.eci_vector,QTForm(qECIToBody,p3.body_vector)))*radToDeg;
  ...
  xPlot(:,k+1) = [x;power;d.tRWA ;angleError3(k+1);bField(:,k);hRWA];
  t            = t + dT;
