function [d,x,p,hRWA ,angleError,torque] =LatLongFunction(x,d,p,jD,lat,long);
 %% LatLong Function for a CubeSat orbit and attitude dynamical model.
%
% Calculates the torque that the reaction wheels have to do to point the 
% desired axis to a desired point in the earth
% 
%   ------
%   Inputs
%   ------
...
radToDeg        = 180/pi;
 ...
 %Latitude Longitude_____
y_latlong=Unit(Cross(v,r));
z_latlong=-Unit(r);
x_latlong=Cross(y_latlong,z_latlong);
M=[x_latlong';y_latlong';z_latlong'];

[c, g, n, p_p] = TruEarth(jD);

g_ef = 6387.165*[cosd(lat)*cosd(long);cosd(lat)*sind(long);sind(lat)];

% p.eci_vector = Unit(g'*g_ef-x(1:3));
p.eci_vector = Unit(n'*g_ef-x(1:3));
