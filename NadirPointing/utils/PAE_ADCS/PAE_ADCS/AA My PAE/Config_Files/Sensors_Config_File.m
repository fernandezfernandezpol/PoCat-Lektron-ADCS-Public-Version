...
Sensors.magnetometer.sigmaNoise=[1, 1, 1]*1.5*10^-6*safeFact;%T  % STD of BIAS Gaussian noise magnetometer 
...
Sensors.gyros.max_scale = 90*pi/180;
...
if Sensors.gyros.on && Sensors.on == 1
    ...
    %Gyro Noise
    Sensors.gyros.Noise= Sensors.gyros.noiseOn*sqrt(Sensors.gyros.arw.^2./dT + 1/12*Sensors.gyros.rrw.^2.*dT).*[normrnd(0,deg2rad(1)),normrnd(0,deg2rad(1)),normrnd(0,deg2rad(1))];%RAD/s


    % w_read = Sensors.gyros.Cerror*x(11:13) + Sensors.gyros.BiasError' + Sensors.gyros.Noise';%Real measurement
    w_read = x(11:13) + Sensors.gyros.BiasError' + Sensors.gyros.Noise'; %Real measurement
...
Sensors.sunSensors.sigmaNoise=0.2*[1, 1, 1, 1, 1, 1]; %Sun sensor noise [V]
% Sensors.sunSensors.sigmaNoise=10^-9*[1, 1, 1, 1, 1, 1]; %Sun sensor noise [V]
