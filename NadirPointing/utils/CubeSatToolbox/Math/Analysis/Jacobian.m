...
%--------------------------------------------------------------------------
%   Copyright (c) 1993-1995 Princeton Satellite Systems, Inc.
%   All rights reserved.
%--------------------------------------------------------------------------
%   Since 1.0
%--------------------------------------------------------------------------
...
if( isempty(tOP) )
   ...
   fOP = feval( funFcn, xOP, tOP, varargin{:} );
end
...
if( isempty(tOP) )
   ...
   for i = 1:m,
      dx     = zeros(m,1);
      dx(i)  = h(i);
      a(:,i) = ( feval(funFcn, xOP+dx, tOP, varargin{:})-feval(funFcn, xOP-dx, tOP, varargin{:}) )/h(i);
   end
end
